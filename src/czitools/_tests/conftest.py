...
@pytest.fixture
def planetable_dict():
    """Return the predefined planetable dictionary with int32 integer columns."""
    int_cols = {"Subblock", "S", "M", "T", "C", "Z", "xstart", "ystart", "width", "height"}
    return {
        k: {i: np.int32(v) for i, v in inner.items()} if k in int_cols else inner
        for k, inner in PLANETABLE_DICT.items()
    }
