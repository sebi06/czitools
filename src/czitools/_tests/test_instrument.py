...
@pytest.mark.parametrize(
    "instrument, expected",
    [
        ({"Microscopes": None}, (None, None, None)),
        (
            {"Microscopes": {"Microscope": {"Id": "Microscope:1"}}},
            ("Microscope:1", None, None),
        ),
    ],
)
def test_microscope_handles_incomplete_metadata(
    instrument: dict, expected: tuple[str | None, str | None, str | None]
) -> None:
    metadata = Box({"ImageDocument": {"Metadata": {"Information": {"Instrument": instrument}}}})

    microscope = czimd.CziMicroscope(metadata)

    assert (microscope.Id, microscope.Name, microscope.System) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("newCZI_zloc.czi", [None, None], [[None], [None], [None], [None]]),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "LLS7_small.czi",
            {
                "name": ["Objective Reflected Light", "Objective Observation"],
                "model": [],
        ...
        (
            "Airyscan.czi",
            {
                "name": ["Plan-Apochromat 63x/1.4 Oil DIC M27"],
                "model": [],
                "immersion": ["Oil"],
