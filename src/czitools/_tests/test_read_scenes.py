"""
Tests for read_scenes and read_6darray_lazy functions in czitools.read_tools.

These tests verify the lazy loading functionality and scene-based reading
...
# ============================================================================
# Tests for read_scenes
# ============================================================================


@pytest.mark.parametrize(
    "czifile, num_scenes_expected, use_dask",
    [
        # Files WITH scenes
...
def test_read_scenes_basic(czifile: str, num_scenes_expected: int, use_dask: bool) -> None:
    """Test basic scene reading functionality for files with scenes."""
    filepath = basedir / "data" / czifile

    arrays, dims, num_scenes = read_tools.read_scenes(filepath, use_dask=use_dask, use_xarray=True)

    # Verify number of scenes
    assert num_scenes == num_scenes_expected

    # Verify we get a list of arrays (one per scene)
    assert isinstance(arrays, list)
    assert len(arrays) == num_scenes_expected
...
def test_read_scenes_xarray_option(czifile: str, use_xarray: bool) -> None:
    """Test read_scenes with and without xarray output."""
    filepath = basedir / "data" / czifile

    arrays, dims, num_scenes = read_tools.read_scenes(filepath, use_dask=False, use_xarray=use_xarray)
...
def test_read_scenes_stack(czifile: str, expected_stacked: bool) -> None:
    """Test read_scenes with stack_scenes option."""
    filepath = basedir / "data" / czifile

    result, dims, num_scenes = read_tools.read_scenes(filepath, use_xarray=True, stack_scenes=True)

    if expected_stacked and num_scenes > 1:
        # Should be a single stacked array, not a list
        assert isinstance(result, xr.DataArray)
        # First dimension should be S (scenes)
        assert "S" in result.dims or result.shape[0] == num_scenes
    else:
        # Single scene case - could be array or list with one element
        assert result is not None


def test_read_scenes_different_shapes() -> None:
    """Test read_scenes with scenes of different shapes (cannot stack)."""
    ...
    arrays, dims, num_scenes = read_tools.read_scenes(filepath, use_xarray=True, stack_scenes=False)

    assert num_scenes > 0
    assert isinstance(arrays, list)
    assert len(arrays) == num_scenes
...
def test_read_scenes_different_shapes_stack_warning() -> None:
    """Test that stacking scenes with different shapes returns a list with warning."""
...
def test_read_scenes_dask_lazy() -> None:
    """Test that dask arrays from read_scenes are truly lazy."""
    filepath = basedir / "data" / "w96_A1+A2.czi"

    arrays, dims, num_scenes = read_tools.read_scenes(filepath, use_dask=True, use_xarray=True)
...
def test_read_scenes_dimensions(czifile: str) -> None:
    """Test that read_scenes returns proper dimension labels."""
    filepath = basedir / "data" / czifile

    arrays, dims, num_scenes = read_tools.read_scenes(filepath, use_xarray=True, use_dask=False)
...
def test_read_scenes_path_object() -> None:
    """Test that read_scenes accepts Path objects."""
    filepath = basedir / "data" / "w96_A1+A2.czi"

    # Pass as Path object (not string)
    arrays, dims, num_scenes = read_tools.read_scenes(filepath)
...
def test_read_scenes_no_scenes_returns_empty() -> None:
    """Test that read_scenes returns empty list for files without scenes."""
