...
def read_6darray_lazy2(
    ...
    Returns:
        Tuple[array6d, mdata]: output as 6D lazy dask array and metadata_tools
    """
    ...
    with pyczi.open_czi(filepath, mdata.pyczi_readertype) as czidoc:
        ...
        for s, time, ch, z in product(
            ...
            image2d = da.from_delayed(
                ...
                shape=(size_y, size_x, 3 if mdata.isRGB else 1),
                dtype=mdata.npdtype[0],
            )

            # insert 2D image plane into the array6d
            array6d[s[0], time[0], ch[0], z[0], ...] = image2d
