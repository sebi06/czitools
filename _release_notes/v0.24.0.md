...
- The `czi_hcs_check` inspector now displays all full-resolution,
  subblock-derived dimension sizes, bounds, valid indices, and the first
  stored scene's Y/X size.
- HCS metadata is filtered to physically stored global scene indices by
  default. The complete XML model remains available as `hcs_declared` or with
  `filter_hcs_to_stored_scenes=False`.
