...
class OnnxInferencer:
    ...
    def predict(self, x: List[np.ndarray]) -> List[np.ndarray]:
        ...
        def _predict_batch(_x: List[np.ndarray], gpu: bool = True) -> List[np.ndarray]:
            """Run prediction on a batch of images.

            Arguments:
                _x: The batch of images to be predicted.
                gpu: Whether or not to allow execution on GPU (True) or enforce CPU execution (False).

            Returns:
                 The predictions for the given batch of images.
            """
            with ManagedOnnxSession(self._model_path, providers=["CUDAExecutionProvider", "CPUExecutionProvider"] if gpu else ["CPUExecutionProvider"]) as sess:
        ...
        return _predict_batch(x, gpu=True)
    ...
    def get_output_shape(self) -> Tuple[Optional[int], Optional[int], Optional[int], Optional[int]]:
        ...
        with ManagedOnnxSession(self._model_path, providers=["CPUExecutionProvider"]) as sess:
            ...
            return cast(Tuple[int, int, int, int], output_shape)
